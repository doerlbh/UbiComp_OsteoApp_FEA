...
# in order to use pigpio, must run 
#       sudo pigpio
# and then to kill the daemon that's running in the background...
#       sudo killal pigpiod
...
def setup_spi(spi) :
    ...
    #spi.max_speed_hz = 19200 # 3.9MHz < 5MHz (highest value for adxl345), 3.9MHz is equivalent to clock divisor of 64
...
fname0 = "debugging/acc0_" + fname + ".pkl"
fname1 = "debugging/acc1_" + fname + ".pkl"
#print fname
f0 = open("debugging/acc0_" + fname + ".pkl", "wb")
f1 = open("debugging/acc1_" + fname + ".pkl", "wb")
...
def read_save0(channel):
    ...
    counter0 += 1
    pickle.dump(axia, f0)
    #f0_list.append(str(axia[0]).zfill(5) + "\t" + str(axia[1]).zfill(5) + "\t" + str(axia[2]).zfill(5) + "\n")
    #f0_string += str(axia[0]).zfill(5) + "\t" + str(axia[1]).zfill(5) + "\t" + str(axia[2]).zfill(5) + "\n"
...
def read_save1(channel):
    ...
    pickle.dump(axia, f1)
    #f1_list.append(axia)
    #f1_string += str(axia[0]).zfill(5) + "\t" + str(axia[1]).zfill(5) + "\t" + str(axia[2]).zfill(5) + "\n"
...
def read_save(spi, f_string):
    ...
    f_string = f_string + str(axia[0]).zfill(5) + "\t" + str(axia[1]).zfill(5) + "\t" + str(axia[2]).zfill(5) + "\n"
        ...
        #GPIO.cleanup()
        #f0 = open("tests/acc0_" + fname + ".csv", "w")
        #f0.write(f0_string)
        #f0.close()
        #f1 = open("tests/acc1_" + fname + ".csv", "w")
        #f1.write(f1_string)
        #f1.close()
        #print("Sampling rate acc0: " + str(sum(1 for line in open("tests/acc0_" + fname + ".csv"))/10) )
        #print("Sampling rate acc1: " + str(sum(1 for line in open("tests/acc1_" + fname + ".csv"))/10) )
        #sys.exit()
...
#f0.write("x_acc\ty_acc\tz_acc\n")
#f1.write("x_acc\ty_acc\tz_acc\n")
...
GPIO.cleanup()
time.sleep(1)
#f0 = open("debugging/acc0_" + fname + ".csv", "w")
#f0.write(f0_string)
f0.close()
#f1 = open("debugging/acc1_" + fname + ".csv", "w")
#f1.write(f1_string)
f1.close()

#print("Sampling rate acc0: " + str(sum(1 for line in open("debugging/acc0_" + fname + ".csv"))/time_run) )
#print("Sampling rate acc1: " + str(sum(1 for line in open("debugging/acc1_" + fname + ".csv"))/time_run) )
...
f0_csv = open("debugging/acc0_" + fname + ".csv", "w")
pickledtext = []
file0 = open(fname0, "rb")
#pickledtext = pickle.load(open(fname1, "rb"))
#for axia in pickledtext :
#    f0_csv.write(str(axia[0]).zfill(5) + "\t" + str(axia[1]).zfill(5) + "\t" + str(axia[2]).zfill(5) + "\n")
l = []
while 1:
    try :
...
f1_csv = open("debugging/acc1_" + fname + ".csv", "w")
pickledtext = []
